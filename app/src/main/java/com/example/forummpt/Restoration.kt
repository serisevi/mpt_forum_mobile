...
class Restoration : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val btnRestSend : MaterialButton = findViewById(R.id.btnRestSend)
        btnRestSend.setOnClickListener { sendCode() }
        currentTheme = Preferences(this@Restoration).getTheme()
        currentNightMode = this.resources.configuration.uiMode and Configuration.UI_MODE_NIGHT_MASK
        checkTheme()
    }

    fun checkTheme() {
        val activity = findViewById<ConstraintLayout>(R.id.activityRestoration)
        when (currentTheme) {
            "Светлая" -> {
    ...
    fun setTextViewsGroupColor(v: View, color : Int) {
        try {
            if (v is ViewGroup) { for (i in 0 until v.childCount) { setTextViewsGroupColor(v.getChildAt(i), color) } }
                ...
                if (v is MaterialButton) { v.setBackgroundColor(Color.parseColor(Preferences(this@Restoration).getAccentColor())) }
            }
        } catch (e: Exception) { e.printStackTrace() }
    }

    fun sendCode() {
        val editRestSend : TextInputEditText = findViewById(R.id.editRestSend)
        val url = getString(R.string.site)+"/api/restoration"
        val email = editRestSend.text.toString()
        if (email.equals("") == false) {
            var formBody : RequestBody = FormBody.Builder().add("email", email).build()
                ...
                override fun onResponse(call: Call?, response: Response) {
                    ...
                    runOnUiThread {
                        if (resp.response.equals("success") == true) {
                            startActivity(Intent(this@Restoration, RestorationCodeRedeem::class.java))
                        }
                        else { Toast.makeText(this@Restoration, "Не удалось найти пользователя", Toast.LENGTH_SHORT).show() }
                    }
                }
